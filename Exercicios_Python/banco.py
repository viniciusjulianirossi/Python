...
# AC03 ADS-EaD - Banco
#
# Email: diego.bueno@aluno.faculdadeimpacta.com.br
...
class Banco():
    ...
    def abre_conta(self, clientes: List[Cliente], saldo_ini: Number) -> None:
        ...
        nova_conta = len(self.__contas) + 1

        c = Conta(clientes, nova_conta ,saldo_ini)   
...
class Conta():
    ...
    def __init__(self, clientes: List[Cliente],
        ...
        self.__transacoes = [("saldo_inicial", saldo_inicial)]
    ...
    def saque(self, valor: Number) -> None:
        ...
        if valor > self.__saldo_atual:
            raise ValueError
        self.__transacoes.append(("saque" , valor))
        self.__saldo_atual = self.__saldo_atual - valor

    def deposito(self, valor: Number):
        '''
        Método depósito da classe Conta, operação deve aparecer no extrato
        '''
        self.__transacoes.append(("deposito" , valor))
        self.__saldo_atual = self.__saldo_atual + valor
